...
get_argv(PyObject *self, PyObject *args)
{
    ...
    for (int i = 0; i < prog_argc; i++) {
        ...
        if (!s) {
            msg(stderr, "cannot convert argument to bytes: %s\n", prog_argv[i]);
            exit(2);
        }
...
{
    PyObject *py_p = Py_None;
    if (orig_env_pythonpath) {
        py_p = PyBytes_FromString(orig_env_pythonpath);
        if (!py_p) {
            msg(stderr, "cannot convert PYTHONPATH to bytes: %s\n",
                orig_env_pythonpath);
            exit(2);
        }
    }
                          ...
                          const char * const relative_path)
{
    ...
    if (rc != 0) {
        msg(stderr, "unable find lib dir (%s): %s\n",
